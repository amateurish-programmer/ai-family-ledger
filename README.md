...
面向 Android 的家庭记账 App，采用 Kotlin + Jetpack Compose + Room。离线记账可直接使用，家庭同步和云端 AI 需要配置自己的 Supabase 项目。
...
- 可配置 Supabase 邮箱登录、家庭邀请、显式同步、冲突处理与打开 App 时同步。
- GitHub Actions 测试、lint、Debug APK 构建与下载产物。

金额仅支持 CNY，以整数分保存。余额变更保留原值、不参与收支。数据库从 V0.1 非破坏性升级，JSON V2 可读取旧版备份。**Supabase 数据库与 AI 函数已部署，DeepSeek 服务端密钥已由用户保存；注册邮件配置及手机/模型联调尚未完成**；连接信息见 [云端服务说明](docs/BACKEND_SETUP.md)。语音识别依赖手机安装的识别服务。构建与验收状态见 [验证记录](docs/VALIDATION.md)。

## 不安装 Android Studio，获取 APK

仓库已创建并配置云端构建。打开上方 **APK 构建与下载 → Run workflow**，保留 main 分支并启动。构建成功后打开该次运行，在 Artifacts 下载 `AI-Family-Ledger-debug-运行序号`，解压并安装 `app-debug.apk`。勾选 `build_only` 只编译 APK；`device_tests` 可另行运行模拟器测试。本轮按用户指示仅构建，不执行功能验收。
...
V0.2 至 V0.5 按原路线连续实现并合并为 0.5.0 交付。2026-09-06 已部署 Supabase 数据库/函数并配置 DeepSeek；剩余外部工作是注册邮件配置、手机填写公开连接参数，以及日后按需进行设备与云端验收；未配置云端不影响本机功能。
