...
金额仅支持 CNY，以整数分保存。余额变更保留原值、不参与收支。数据库从 V0.1 非破坏性升级，JSON V2 可读取旧版备份。**云端代码已包含，实际 Supabase 和大模型服务尚未部署或联调**；配置见 [云端服务说明](docs/BACKEND_SETUP.md)。语音识别依赖手机安装的识别服务。构建与验收状态见 [验证记录](docs/VALIDATION.md)。
...
V0.2 至 V0.5 按原路线连续实现并合并为 0.5.0 交付。剩余外部工作是创建 Supabase 项目、部署 SQL/函数、配置模型供应商，以及日后按需进行设备与云端验收；未配置云端不影响本机功能。
